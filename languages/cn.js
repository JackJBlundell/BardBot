...
module.exports = {
  ...
  LANGUAGE: (newlangstring) =>
    `${Emojis.check.str} 将语言更改为 **${newlangstring}**`,
  TIME_ENDED: `Time ran out`,

  JOIN_VC: `${Emojis.warn.str} **请先加入频道**`,
  ALREADY_CONNECTED: (channelId) =>
    `${Emojis.denied.str} **我已经连接了 ${parseChannelMention(channelId)}**!`,
  MISSING_PERMS: (permString) =>
    `${Emojis.denied.str} **我在您的语音频道中缺少 “${permString}” 的权限！**`,

  COULD_NOT_JOIN: (channelId) =>
    `${Emojis.cross.str} **我无法连接到 ${parseChannelMention(channelId)}。**`,
  HELP: (prefix) => {
    return newLiner(
  ...
  NOT_CONTROLLING: (prefix) =>
    `${Emojis.cross.str} **您不是通过 \`${prefix}control\` 控制机器人的人**`,
  FILTER: (state, filter) =>
    `🎚 **成功 ${state ? "添加" : "移除"} \`${filter}\` 过滤器。**`,
