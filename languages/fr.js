...
module.exports = {
  ...
  PREFIXINFO: (prefix) =>
    `${Emojis.check.str} **Mon préfixe ici est \`${prefix}\`**`,

  LANGUAGE: (newlangstring) =>
    `${Emojis.check.str} Changement de la langue en **${newlangstring}**`,
  TIME_ENDED: `Time ran out`,

  JOIN_VC: `${Emojis.warn.str} **Veuillez d'abord rejoindre une chaîne**`,
  ALREADY_CONNECTED: (channelId) =>
    `${Emojis.denied.str} **Je suis déjà connecté à ${parseChannelMention(
      channelId
    )}**!`,
  MISSING_PERMS: (permString) =>
    `${Emojis.denied.str} **Il me manque l'autorisation de "${permString}" dans votre canal vocal!**`,

  COULD_NOT_JOIN: (channelId) =>
    `${Emojis.cross.str} **Je n'ai pas pu me connecter à ${parseChannelMention(
      channelId
    )}.**`,
  HELP: (prefix) => {
    return newLiner(
  ...
  NOT_CONTROLLING: (prefix) =>
    `${Emojis.cross.str} **Vous n'êtes pas celui qui contrôle le bot via \`${prefix}control\`**`,
  FILTER: (state, filter) =>
    `🎚 **${state ? "ajouté" : "supprimé"} le \`${filtre}\` Filtre.**`,
