...
module.exports = {
  ...
  PREFIXINFO: (prefix) =>
    `${Emojis.check.str} **Mein Prefix lautet: \`${prefix}\`**`,

  LANGUAGE: (newlangstring) =>
    `${Emojis.check.str} Sprache auf **${newlangstring}** geändert!`,
  ...
  ALREADY_CONNECTED: (channelId) =>
    `${Emojis.denied.str} **Ich bin schon in ${parseChannelMention(
      channelId
    )} verbunden**!`,
  MISSING_PERMS: (permString) =>
    `${Emojis.denied.str} **Mir fehlen die Berechtigung für "${permString}" in deinem Voice-Channel!**`,

  COULD_NOT_JOIN: (channelId) =>
    `${Emojis.cross.str} **Ich konnte ${parseChannelMention(
      channelId
    )} nicht beitreten.**`,
  HELP: (prefix) => {
    return newLiner(
  ...
  NOT_CONTROLLING: (prefix) =>
    `${Emojis.cross.str} **Du kontrollierst nicht den Bot mit: \`${prefix}control\`**`,
  FILTER: (state, filter) =>
    `🎚 **Der Filter \`${filter}\` wurde erfolgreich ${
      state ? "hinzugefügt" : "entfernt"
    }.**`,
