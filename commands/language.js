      ...
      if (langs.length > i) {
        allcomps.push(
          new ActionRowBuilder().addComponents(
            langs.slice(i, i + 5).map(([l, data]) =>
              new ButtonBuilder()
                .setStyle(
                  lang == l ? ButtonStyle.Primary : ButtonStyle.Secondary
                )
                .setDisabled(lang == l)
                .setLabel(data.text)
                .setEmoji(data.emoji)
                .setCustomId(`language_${l}`)
            )
          )
        );
      }
        ...
        collector.on("collect", async (i) => {
          ...
          i.update({
            ...
            content: translate(
              ...
              message.guildId,
              "LANGUAGE",
              `${languageStrings[newlang].emoji} ${languageStrings[newlang].text}`
            ),
          });
        });
